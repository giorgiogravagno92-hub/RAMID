...
if (smtpHost && smtpUser && smtpPass) {
  ...
  console.warn('[MAILER] SMTP non configurato. Le email verranno stampate solo sulla console del server.');
}
...
export const sendVerificationEmail = async (toEmail: string, verificationLink: string) => {
  ...
  if (transporter) {
    try {
      ...
      console.log(`[MAILER] Email di verifica inviata con successo a ${toEmail}`);
    } catch (error) {
      console.error(`[MAILER] Errore durante l'invio dell'email a ${toEmail}:`, error);
